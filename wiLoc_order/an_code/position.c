...
double score_cal(AP_SCORE *score,AP_info *info,char *path,char *suffix,int n,int m,char *result)
{
	DIR *dirp;
	struct dirent *dir;	
	FILE *fp;
...
int wiLoc_position(AP_info *ap,int apn,char *result){
	...
	printf("cacluate postion request!\n");

	int nidx=dirn("./idx",".idx");
...
*//////////////////////////////////////
	...
	result[0]='1';
	double mymax=score_cal(score,info,"./idx/",".idx",nidx,apn,result);
	printf("max:%s,%f\n",result+1,mymax);	
	for(i=0;i<nidx;i++)
	{
